...
namespace WebServicePOS
{
    ...
    public class vfpPOS : IDisposable
    {
        ...
        public vfpPOS()
        {
            ...
            dbfConn.ConnectionString = "Provider=vfpoledb.1;Data Source=" + @"E:\(MENTOR)\#DATE\E\EXCELSIO\" + ";Collating Sequence=general;";
            try
            {
                dbfConn.Open();
            }
        ...
        public DataTable getSales(DateTime initialPeriod, DateTime finalPeriod)
        {
            ...
            String command = @"SELECT fa.nrdoc, fa.data, fa.fuben, fb.denfb," +
                                    " fa.gest, fa.agent, fp.codp, fa.cant, fp.denm, fa.tvac," +
        ...
        {
            String command = @"SELECT st.gest,st.codp,st.denp,st.pv,st.cant FROM dbf\stocreal AS st LEFT JOIN nom\fp ON st.codp == fp.codp
                                        WHERE fp.fuben == 'G292'";
            oCmd.CommandText = command;
            DataTable dt = new DataTable();
            dt.Load(oCmd.ExecuteReader());
